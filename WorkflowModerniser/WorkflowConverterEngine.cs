...
{
	internal class WorkflowConverterEngine<TEntityVariable> : IWorkflowConverter where TEntityVariable : EntityVariable
	{
		internal WorkflowConverterEngine(IWorkflowSource workflowSource, IMetadataService metadataService, Func<WriterContext, IWorkflowWriter<TEntityVariable>> writerFactory)
		{
		...
		public IEnumerable<IOutput> Convert(Guid workflowId)
		{
			IWorkflow workflow = this.workflowSource.GetWorkflow(workflowId);
			...
			switch (workflow.Category)
			{
				case Workflow_Category.BusinessRule:
					{
						WriterContext context = new WriterContext(workflow.Name, false, MessageName.Action, workflow.PrimaryEntity, metadataService);
				...
				case Workflow_Category.Workflow:
					{
						...
						foreach (Workflow_Stage stage in new[] { Workflow_Stage.PreOperation, Workflow_Stage.PostOperation })
						{
							...
							{
								WriterContext context = new WriterContext(workflow.Name, stage == Workflow_Stage.PreOperation, thisOperationMessages, workflow.PrimaryEntity, metadataService);
								results.AddRange(Convert(workflow, context));
							}
						}
		...
		private void ConvertStartChildWorkflow(StartChildWorkflow startChildWorkflow)
		{
			...
			IEnumerable<IOutput> outputs = new WorkflowConverterEngine<TEntityVariable>(workflowSource, metadataService, writerFactory).Convert(workflowId);
